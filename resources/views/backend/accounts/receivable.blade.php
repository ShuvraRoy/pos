...
@section('page_header','Cuentas por Cobrar')
@section('page_links')
...
@section('page_content')
    ...
    <div class="panel panel-primary" data-collapsed="0">
        <form action="{{url('accounts_receivable/date_filter')}}" method="post">
            @csrf
        ...
        <div class="table-responsive">
            <table class="table table-bordered datatable" id="receivable_accounts_table">
                <thead>
...
@section('page_scripts')
    ...
    <script type="text/javascript">
        ...
        function initialise_table() {
            var receivable_accounts_table = jQuery("#receivable_accounts_table");
            let from = $("input[name=from_date]").val();
            let to = $("input[name=to_date]").val();
            receivable_accounts_table.DataTable({
                ...
                "ajax": {
                    "type": 'POST',
                    url:'{{ url("accounts_receivable/get_receivable_accounts_data") }}',
                    "data" : {
                        "_token": "{{ csrf_token() }}",
                        from:from,
                        to:to
                ...
                buttons: [
                    {
                        ...
                        title: "Receivable Accounts Report",
                        exportOptions: {
                            columns: [0, 1, 2, 3 ]
                        }
                    },
            ...
            receivable_accounts_table.closest('.dataTables_wrapper').find('select').select2({
